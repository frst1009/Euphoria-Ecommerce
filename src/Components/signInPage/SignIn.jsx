...
const Signin = () => {
  ...
  const validationschema = Yup.object().shape({
    name: Yup.string().email(t("email")).required(t("email_formik")),
    password: Yup.string().required(t("password_formik")).min(6, t("pass_min")),
  });
              ...
              {({ isSubmitting }) => (
                <Form className="pt-4">
                  <div className="flex flex-col mb-3">
                    <label className="pb-1 md:text-xl  lg:text-sm">
                      {t("username")}
                    </label>
                    <Field
                      type="email"
                      name="name"
                      className=" rounded-md py-2 px-2 border border-gray-500 min-w-100 "
                    />
                    <ErrorMessage
                      name="name"
                  ...
                  <div className="input password mb-3 flex flex-col">
                    <div className="labels flex justify-between items-center">
                      ...
                      <label
                        htmlFor="password"
                        className="flex items-center md:g-5 cursor-pointer md:text-xl lg:text-sm"
                        onClick={handlePass}
                      >
                        {icon} {t("hide")}{" "}
                      </label>
